...
3.  How many variables are in the data set? **Answer**: 8 variables.
4.  How many observations are in the data set? **Answer:** 344 observations
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>) **Answer**: 3 data types (considering that int and dbl are different)
...
```{r}
penguins_2900<- penguins |> filter (body_mass_g <= "2900")
```
...
```{r}
penguins_adelie_40<- penguins_adelie |> filter (bill_length_mm > 40)
```
...
penguins_no_chinstraps<- penguins |> filter (species != "Chinstrap")
```
...
```{r}
penguins_gentoo_15_5<- penguins |> filter (species == "Gentoo", bill_depth_mm > 15.5)
```
...
```{r}
penguins_male_dream_biscoe<- penguins |> filter (sex == "male", island == "Dream"| island == "Biscoe")
```
...
```{r}
penguins_female_adelie_35<- penguins_adelie |> filter (sex == "female", bill_length_mm < 35)
```
...
```{r}
penguins_female_chinstraps<- penguins |> filter (species == "Chinstrap", sex == "female", island == "Dream" | island == "Torgersen")
```
...
```{r}
penguins_gentoo_4500<- penguins |> filter (species == "Gentoo" | body_mass_g > 4500)
```
...
```{r}
penguins <- penguins |> mutate(id = 1:n()) |> relocate (id)
glimpse(penguins)
```

## Task 3: Summarize data with group_by() and summarise() & count()
...
penguins |> group_by(species) |> 
  filter(!is.na(flipper_m))|> 
  summarise(max = max(flipper_m), min = min (flipper_m), n = n())
```
...
penguins |> group_by(species, year) |> 
  filter(!is.na(flipper_m))|> 
  summarise(bill_depth_mean = mean(bill_depth_mm), bill_length_mean = mean(bill_length_mm), n = n())
```
...
penguins_female_torgersen_biscoe<- penguins |> filter ( sex == "female", island == "Biscoe" | island == "Torgersen") 
penguins_female_torgersen_biscoe|> count(sex, island)
```
